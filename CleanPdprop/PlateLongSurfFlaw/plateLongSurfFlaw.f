C  plateLongSurfFlaw.f   vers. 4.2   Notched Spec Crack Prop.  Aug 23 2018
      SAVE
...
C
C vers 4.2 Fixed 2nd pt read bug in s/r getPeakLoads()   Aug 23 2018
...
C Fork to  plateLongSurfFlaw.f  from plateWeldflaw.f   Dec.31 2012 vers. 2.2
...
C     Matrices for storing Mm and Mb
      real*4 storeLongMm(1000),storeLongMb(1000)
      real*4 deltaLongaob,aobmin,aobmax
      integer maxstore
      common /LONGMMDATA/storeLongMm,storeLongMb,deltaLongaob,aobmin,
     &        aobmax,maxstore
      logical debugMm,lactivateMmMb

C     Matrices for storing Mkm and Mkb
      real*4 storeLongMkm(1000),storeLongMkb(1000)
      real*4 deltaLongzob,zobmin,zobmax,wLoB
      integer maxstore2
      real*4 Lweld
      common /LONGMKDATA/storeLongMkm,storeLongMkb,deltaLongzob,zobmin,
     &        zobmax,maxstore2,wLoB
...
C---------------------------  Run time input data------------------
  184 continue
      write(6,185)
      write(0,185)
  185 format("# plateLongSurfFlaw.f vers. 4.2"/
     & "#Usage: plateLongSurfFlaw  scale <histfile  >outfile"/)

      nargc = iargc()
C     Note that in HP fortran the arg numbers must be changed by -1
C     and that iargc probably includes the "plateLongSurfFlaw" as an arg.
      if( nargc .ne. 1)then
        write(0,*)" plateLongSurfFlaw:  usage ERROR"
        write(6,*)" plateLongSurfFlaw:  usage ERROR"
        write(0,186)
        write(6,186)
  186  format(
     &   "#Usage:      plateLongSurfFlaw Scale <histfile ",
     &   " >outputFile"//
...
C        Ok, its a number
  ...
  716      format("# Error too many data points:",i5,
     &        " recompile plateLongSurfFlaw.f or reduce matfile data")
           stop
         endif
...
C        Ok, its a number
  ...
  616      format(" Error too many data points:",i5,
     &       " recompile plateLongSurfFlaw.f or reduce dadnTable data")
           stop
         endif
...
C-------------- Initilize the Mm,Mb, Mkm,Mkb storage -----------------------------
      ...
      if(lactivateMmMb)then
         call getLongMmMb(ivec,iret,aob,xMm,xMb)
         if(iret.ne.0)then
           write(0,*)"# Error from getLongMmMb() Init., Stopping..."
           stop
         endif
      endif

      wLoB=Lweld/Bthick  !wLoB is in common
      aob=0.2   !  set to something
      if(lactivateMkmMkb)then
         call getLongMkmMkb(ivec,iret,aob,xMkm,xMkb)
         if(iret.ne.0)then
           write(0,*)"#Error: getLongMkmMkb() Initilize. Stopping."
           stop
         endif
      endif
...
C        Ok, its a number
  ...
  916      format(" Error too many data points:",i5,
     &      "recompile plateLongSurfFlaw.f or reduce load history data")
           stop
         endif
...
C     Ok,  previous copies do not exist. open it.
     ...
     &     form= "unformatted", status= "new", recl= 48)
      write(0,462)
      write(6,462)
...
C     total damage is large.
  ...
  119 format("#crk= #   NREV    a 90      NBLK    NACT    ",
     &       "lobj90    xMm90,xMb90,xMkm90,xMkb90, nptt90")
C      write(6,120) nrev,totdam90,nblk,nact,lobj90,xMm90,xMb90,
C     &             xMkm90,xMkb90,nptt90
...
Cold      lobj= lobj*xkp(totdam,nrev)
 ...
 3002    format("#END: a/B > 0.6 : a= ",e14.7," B= ",e14.7)
         goto 9000   !stop
      endif

      if(lactivateMmMb)then ! if not, all are equal to 1.0 (set above)
        call getLongMmMb(ivec,iret,aob,xMm90,xMb90)
        if(iret .ne. 0)then
          goto 9000  !stop
        endif
      endif
      ...
      if(lactivateMkmMkb)then ! if not, all are equal to 1.0 (set above)
        call getLongMkmMkb(ivec,iret,aob,xMkm90,xMkb90)
      ...
      xfw=1.0    ! See BS7910 2005  M.3.3
C        call fwSurfFlaw(ivec,iret,cow,aob,xfw)
...
C     We always write to the binary out file. RecSize is 12*4 =48
     ...
     &             lobj90,xMm90,xMb90, xMkm90,xMkb90,xfw,
...
C       Ok, Point(idat) on the curve is still smaller, keep going
 ...
 1000   continue
        ...
        write(0,*)"ERROR:plateLongSurfFlaw:getStress2Strain.endofdoloop"
        iexit=1
...
C      end of hunting loop
 1000  continue
        ...
        write(0,*)" ERROR:plateLongSurfFlaw:getloop.endofdoloop"
        iexit=1
...
C     Get the second point   stspres  to create the initial line.
  100 continue
      n=n+1
Cbug  Fix 2nd pt read bug Aug 2018
...
C==============================================================
      ...
      SUBROUTINE getLongMmMb(ivec,iret,aob,xMm,xMb)
      SAVE
C     ivec=0  initilize storages, ivec=1  compute xMm and xMb
C     Plate Long (full width) Surface crack
C     We have 1000 points to define the curve, so don't bother interpolating,
C     just take nearest value as a solution to   aob  input.
C     Use equations  M.11 and M.12 from  BS7910 2005
     ...
     &        aobmax,maxstore
      ...
      if(ivec.lt.0 .or. ivec .gt.2)then
        write(0,110)ivec
        write(6,110)ivec
  110   format("#getLongMmMb Error: ivec=",i6," unknown."/"#Stopping")
        stop
      endif
...
C     if there are maxstore  storage points, then delta must be 
      deltaLongaob= (aobmax-aobmin)/(float(maxstore-1) )
      n=1
      aob=aobmin
      write(6,290)
  290 format("#longMmMb= #  a/B      Mm       Mb        n")

  300 continue
      aob2=aob*aob
      aob3=aob2*aob
      aob4=aob3*aob
      storeLongMm(n)=1.12 -0.23*aob +10.6*aob2 -21.7*aob3 +30.4*aob4
      storeLongMb(n)=1.12 -1.39*aob +7.32*aob2 -13.1*aob3 +14.0*aob4
      write(6,320)aob,storeLongMm(n),storeLongMb(n),n
  320 format("#longMmMb= ",f8.6,1x,f8.6,1x,f8.6,1x,i5)
      n=n+1
      if(n.gt.maxstore)return
C     keep going
      aob=aob+deltaLongaob
      goto 300

  500 continue
C     Get a solution
      if(aob.gt.aobmax)then
        write(0,520)aob,aobmax
        write(6,520)aob,aobmax
  520   format("#longMmMb= #END:  a/B= ",f8.6," .gt.  a/B max= ",f8.6)
        iret=999
        return   !stop
      endif
      if(aob.lt.aobmin)then
        write(0,530)aob,aobmin
        write(6,530)aob,aobmin
  530   format("#longMmMb= #ERROR:  a/B= ",f8.6," .lt.  a/B min= ",f8.6)
        itret=999
        return  !stop
      endif

      n= ifix(aob/deltaLongaob)+1
...
C     so don't bother. Just take nearest.
      ...
      if(ivec .eq.2)then   !debug output
        write(0,550)aob,xMm,xMb
        write(6,550)aob,xMm,xMb
  550   format("#lognMmMb= ",3(1x,f8.6),"#debug a/B xMm xMb")
      endif
      return
      end

C===========================================================================
      SUBROUTINE getLongMkmMkb(ivec,iret,zob,xMkm,xMkb)
      SAVE
C     ivec=0  initilize storages, ivec=1  compute xMm and xMb.
C     ivec=2  Turn on debug output.
C     Plate Long (full width) Surface crack near Weld.  2D
C     We have 1000 points to define the curve, so don't bother interpolating,
C     just take nearest value as a solution to   zob  input.
C     Use equations  eq. M26 and Table M.9  from  BS7910 2005

      real*4 storeLongMkm(1000),storeLongMkb(1000)
      real*4 deltaLongzob,zobmin,zobmax,wLoB
      integer maxstore
      common /LONGMKDATA/storeLongMkm,storeLongMkb,deltaLongzob,zobmin,
     &        zobmax,maxstore,wLoB
      ...
      if(ivec.lt.0 .or. ivec .gt.2)then
        write(0,110)ivec
        write(6,110)ivec
  110   format("#getLongMkmMkb Error: ivec=",i6," unknown."/"#Stopping")
        stop
      endif
...
C     ivec == 0   initilize storage
      ...
      deltaLongzob= zobmax/float(maxstore)
      zobmin=deltaLongzob     ! equations don't work for zob=0.0
C     The first storage is for zob= zobmax/maxstore
      n=1
      zob=zobmin

C     Precompute some common stuff in table.
      wLoBp55= wLoB**(0.55)
      wLoBp21= wLoB**(0.21)
      wLoBp27= wLoB**(0.27)
      wLoBp46= wLoB**(0.46)
      ...
      write(6,280)wLoB
  280 format("#longMkmMkb= #L/B= ",e14.7,"  #Initilize Storage...")
      write(6,290)
  290 format("#longMkmMkb= #  z/B      Mkm       Mkb     L/B      n")

  300 continue

C     Compute the Mkm  Axial loading - - - - - - - - - - - - - 
      if(wLoB .le. 2.0)then
         if(zob .le. (0.05*wLoBp55) )then
           v= 0.51*wLoBp27
           w= -0.31
           goto 340
         endif
C        no, then zob .gt. 0.05*wLoBp55
           v= 0.83
           w= -0.15*wLoBp46
           goto 340
...
C     no,  then wLoB  .gt. 2.0
         if(zob .le. 0.073)then
           v= 0.615
           w= -0.31
           go to 340
         endif
C     no, then  zob .gt. 0.073
           v=  0.83
           w= -0.20
...
C     Compute the Mkb  Bending loading factor - - - - - - - - - - -
  400 continue
      if(wLoB .le. 1.0)then
         if(zob .le. (0.03*wLoBp55) )then
           v= 0.45*wLoBp21
           w= -0.31
           goto 440
         endif
C        no, then zob .gt. 0.03*wLoBp55
           v= 0.68
           w= -0.19*wLoBp21
           goto 440
...
C     no,  then wLoB  .gt. 1.0
         if(zob .le. 0.03)then
           v= 0.45
           w= -0.31
           go to 440
         endif
C     no, then  zob .gt. 0.03
           v=  0.68
           w= -0.19
  ...
  440    continue
      ...
      storeLongMkm(n)=xMkm
      storeLongMkb(n)=xMkb
      write(6,460)zob,xMkm,xMkb,wLoB,n
  460 format("#longMkmMkb= ",f8.6,2(1x,f9.6),1x,f6.3,1x,i5)

C     step to next n and zob
      n=n+1
      zob=zob+deltaLongzob
      if(n.le.maxstore)goto 300
      iret=0
      return

C     Use the storage values to compute xMkm and xMkb from zob
C     Remember that the first store contains values for zob=0.0+deltaLongzob
  500 continue
      if(zob .lt. zobmin)then
        write(0,550)zob,zobmin
  550   format("#longMkmMkb= #Error: z/B= ",f8.6," .lt. z/B min.=",f8.3)
        iret=1
        return
      endif
      if(zob .gt.zobmax)then
        write(0,555)zob,zobmin
  555   format("#longMkmMkb= #END: z/B= ",f8.6," .gt. z/B max.=",f8.3)
        iret=1
        return
      endif
C     ok,  within range of data
      n=ifix(zob/deltaLongzob)
      if(n.eq.0)n=1   ! equation not valid near zob= 0.0  (log-log), thus
C                       use next value available at store(1)
      xMkm=storeLongMkm(n)
      xMkb=storeLongMkb(n)
      if(ivec .eq. 2)then  ! debug
        write(6,560)zob,xMkm,xMkb,wLoB,n
  560   format("#longMkmMkb= #Debug:",f8.6,2(1x,f9.6),1x,f6.3,1x,i5)
      endif
      iret=0
      return
      end
