C  plateEdgeFAD.f   vers. 0.60     FAC aug.28 2015
      SAVE
...
C  Usage:   plateEdgeFAD    >outputFile      (done by makereport5 )
...
C vers. 0.60  Re-write indexing code for plot intervals   Aug 28 2015
C             ( revisions begin near statement 1012 )
...
C  The numbers were printed by plateEdgeSurfFlaw.f  statement:
C        nrecord=nrecord+1
...
C     Storage for the FAD  diagram points
      ...
      write(0,180)
      write(6,180)
  180 format("# plateEdgeFAD.f  vers. 0.6")
  190 continue
      write(6,191)
      write(0,191)
  ...
  715     format("#FAD ",f7.5,1x,f8.4," #FAD1")
        ...
        if(ctypefad .eq. "#FAD2a" )then
          ...
          if(nfad2a .gt. maxfads)then
             write(0,716)
             write(6,716)
  716        format("#Error: plateEdgeFAD: too many FAD2a data points."/
     &            "# you need to edit your FAD table file, or "
     &            " recompile the program plateEdgeFAD.f   Stopping...")
             stop
          endif
  ...
  718     format("#FAD ",f7.5,1x,f8.4," #FAD2a")
        ...
        if(ctypefad .eq. "#FAD2b" )then
          ...
          if(nfad2b .gt. maxfads)then
             write(0,721)
             write(6,721)
  721        format("#Error: plateEdgeFAD: too many FAD2b data points."/
     &            "# you need to edit your FAD table file, or "
     &            " recompile the program plateEdgeFAD.f   Stopping...")
           stop
          endif
...
C         There are not that many recs items in this history.
 ...
 1015     format("# Warning! plateEdgeFAD: Max Recs ",i10," < 1000  ",
...
C     Due to roundoff, there may be extra recs. past last interval
 ...
 1020 format("#plateEdgeFAD: For plotting: "/
...
C        Read in the first rec of the interval to set the max mins
     ...
     &             stsMembrane,stsBending
        ...
        if(jrecStart .eq. jrecEnd)then ! happens when remainder=1
           jrec=jrecEnd
           write(0,*)"#plotEdgeFAD: jrecStart=jrecEnd= ",jrecStart
           write(6,*)"#plotEdgeFAD: jrecStart=jrecEnd= ",jrecStart
C          This was read in up above.
C           read(60,rec=jrec)nrev,totdam90,nblk,nact,
C     &             lobj90,xMm90,xMb90,
C     &             stsMembrane,stsBending
C           xlobj90max=lobj90
C           xlobj90min=lobj90
...
C           stsBmin=stsBending
           goto 1901 !skip the interval scan, there is just 1 record.
        endif
...
C       Ok, its a regular set of interval points or a set of remainders
 ...
 1901    continue  !come here if remainder =1 and no loop was required
...
C        compute  Kr, Lr, and the KrEOL, and LrEOL
         ...
         if(interval .gt. maxIntervals+1)then
           write(0,*)"#plateEdgeFAD: Plot Interval Storage Error: ",
     &     "interval,jrecStart,jrecEnd,jrec,maxIntervals= ",
     &     interval, jrecStart,jrecEnd,jrec,maxIntervals
           stop
         endif
         amaxS(interval)=amax
...
C        See BS7910 2005  Annex P.3.3 page 240
C        (same as plateSurf flaw except for   a2prime   calc. )
           a2prime=(amax/Width)
           term2= 3.0 *( (1.0-a2prime)**2 )

C        From BS7910 P.3.5 pg 241    pinjointed is not relevant
