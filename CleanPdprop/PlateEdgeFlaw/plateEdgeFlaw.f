C  plateEdgeFlaw.f   vers. 3.10   Plate Edge Crack Prop.  FAC nov 02 2013
      SAVE
...
C           In BS7910:  M=fw=1,  Mkm,Mkb not used.
...
C     Matrices for storing Mm and Mb
      real*4 storeEdgeMm(1000),storeEdgeMb(1000)
      real*4 deltaEdgeaow,aowmin,aowmax
      integer maxstore
      common /LONGMMDATA/storeEdgeMm,storeEdgeMb,deltaEdgeaow,aowmin,
     &        aowmax,maxstore
      logical debugMm,lactivateMmMb

CC     Matrices for storing Mkm and Mkb
C      real*4 storeEdgeMkm(1000),storeEdgeMkb(1000)
C      real*4 deltaEdgezob,zobmin,zobmax,wLoB
C      integer maxstore2
C      real*4 Lweld
C      common /LONGMKDATA/storeEdgeMkm,storeEdgeMkb,deltaEdgezob,zobmin,
...
C---------------------------  Run time input data------------------
  184 continue
      write(6,185)
      write(0,185)
  185 format("# plateEdgeFlaw.f vers. 3.10"/
     & "#Usage: plateEdgeFlaw  scale <histfile  >outfile"/)

      nargc = iargc()
C     Note that in HP fortran the arg numbers must be changed by -1
C     and that iargc probably includes the "plateEdgeFlaw" as an arg.
      if( nargc .ne. 1)then
        write(0,*)" plateEdgeFlaw:  usage ERROR"
        write(6,*)" plateEdgeFlaw:  usage ERROR"
        write(0,186)
        write(6,186)
  186  format(
     &   "#Usage:      plateEdgeFlaw Scale <histfile ",
     &   " >outputFile"//
...
C        Ok, its a number
  ...
  716      format("# Error too many data points:",i5,
     &        " recompile plateEdgeFlaw.f or reduce matfile data")
           stop
         endif
...
C        Ok, its a number
  ...
  616      format(" Error too many data points:",i5,
     &       " recompile plateEdgeFlaw.f or reduce dadnTable data")
           stop
         endif
...
C-------------- Initilize the Mm,Mb, Mkm,Mkb storage -----------------------------
      ...
      if(lactivateMmMb)then
         call getEdgeMmMb(ivec,iret,aow,xMm,xMb)
         if(iret.ne.0)then
           write(0,*)"# Error from getEdgeMmMb() Init., Stopping..."
           stop
         endif
      endif

C      wLoB=Lweld/Bthick  !wLoB is in common
C      aow=0.2   !  set to something
C      if(lactivateMkmMkb)then
C         call getEdgeMkmMkb(ivec,iret,aow,xMkm,xMkb)
C         if(iret.ne.0)then
C           write(0,*)"#Error: getEdgeMkmMkb() Initilize. Stopping."
C           stop
C         endif
C      endif
...
C        Ok, its a number
  ...
  916      format(" Error too many data points:",i5,
     &      "recompile plateEdgeFlaw.f or reduce load history data")
           stop
         endif
...
Cold      lobj= lobj*xkp(totdam,nrev)
 ...
 3002    format("#END: a/W > 0.6 : a= ",e14.7," W= ",e14.7)
         goto 9000   !stop
      endif

      if(lactivateMmMb)then ! if not, all are equal to 1.0 (set above)
        call getEdgeMmMb(ivec,iret,aow,xMm90,xMb90)
        if(iret .ne. 0)then
          goto 9000  !stop
        endif
      endif
...
C        call getEdgeMkmMkb(ivec,iret,aow,xMkm90,xMkb90)
...
C        call fwSurfFlaw(ivec,iret,cow,aow,xfw)
...
C       Ok, Point(idat) on the curve is still smaller, keep going
 ...
 1000   continue
        ...
        write(0,*)"ERROR:plateEdgeFlaw:getStress2Strain.endofdoloop"
        iexit=1
...
C      end of hunting loop
 1000  continue
        ...
        write(0,*)" ERROR:plateEdgeFlaw:getloop.endofdoloop"
        iexit=1
...
C==============================================================
      ...
      SUBROUTINE getEdgeMmMb(ivec,iret,aow,xMm,xMb)
      SAVE
C     ivec=0  initilize storages, ivec=1  compute xMm and xMb
C     Plate Edge (full width) Surface crack
C     We have 1000 points to define the curve, so don't bother interpolating,
C     just take nearest value as a solution to   aow  input.
C     Use equations  M.11 and M.12 from  BS7910 2005
     ...
     &        aowmax,maxstore
      ...
      if(ivec.lt.0 .or. ivec .gt.2)then
        write(0,110)ivec
        write(6,110)ivec
  110   format("#getEdgeMmMb Error: ivec=",i6," unknown."/"#Stopping")
        stop
      endif
...
C     if there are maxstore  storage points, then delta must be 
      deltaEdgeaow= (aowmax-aowmin)/(float(maxstore-1) )
      n=1
      aow=aowmin
      write(6,290)
  290 format("#edgeMmMb= #  a/B      Mm       Mb        n")

  300 continue
      aow2=aow*aow
      aow3=aow2*aow
      aow4=aow3*aow
C     According to BS7910  Mm=Mb=
      storeEdgeMm(n)=1.12 -0.23*aow +10.6*aow2 -21.7*aow3 +30.4*aow4
      storeEdgeMb(n)=storeEdgeMm(n)
      write(6,320)aow,storeEdgeMm(n),storeEdgeMb(n),n
  320 format("#edgeMmMb= ",f8.6,1x,f8.6,1x,f8.6,1x,i5)
      n=n+1
      if(n.gt.maxstore)return
C     keep going
      aow=aow+deltaEdgeaow
      goto 300

  500 continue
C     Get a solution
      if(aow.gt.aowmax)then
        write(0,520)aow,aowmax
        write(6,520)aow,aowmax
  520   format("#edgeMmMb= #END:  a/W= ",f8.6," .gt.  a/W max= ",f8.6)
        iret=999
        return   !stop
      endif
      if(aow.lt.aowmin)then
        write(0,530)aow,aowmin
        write(6,530)aow,aowmin
  530   format("#edgeMmMb= #ERROR:  a/W= ",f8.6," .lt.  a/W min= ",f8.6)
        itret=999
        return  !stop
      endif

      n= ifix(aow/deltaEdgeaow)+1
...
C     so don't bother. Just take nearest.
      ...
      if(ivec .eq.2)then   !debug output
        write(0,550)aow,xMm,xMb
        write(6,550)aow,xMm,xMb
  550   format("#edgeMmMb= ",3(1x,f8.6),"#debug a/W xMm xMb")
      endif
      return
      end

C===========================================================================
C      SUBROUTINE getEdgeMkmMkb(ivec,iret,zob,xMkm,xMkb)
C      BS7910 does not presently account for welds in an edge cracked plate.
