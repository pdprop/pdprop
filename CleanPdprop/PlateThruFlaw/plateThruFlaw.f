C  plateThruFlaw.f   vers. 3.11   Notched Spec Crack Prop.  nov 21 2013
      SAVE
...
C     Vector for storing fw
      real*4  fw(1000)
      integer*4 maxfwdatarow   ! storage limit
      real*4  startaow,endaow,deltaaow  ! data limits, incrm.
      logical debugfw,lactivatefw
      common/FWDATA/ fw, maxfwdatarow,startaow,endaow,deltaaow,
     &              debugfw,lactivatefw
...
C---------------------------  Run time input data------------------
  184 continue
      write(6,185)
      write(0,185)
  185 format("# plateThruFlaw.f vers. 3.11"/
     & "#Usage: plateThruFlaw  scale <histfile  >outfile"/)

      nargc = iargc()
C     Note that in HP fortran the arg numbers must be changed by -1
C     and that iargc probably includes the "plateThruFlaw" as an arg.
      if( nargc .ne. 1)then
        write(0,*)" plateThruFlaw:  usage ERROR"
        write(6,*)" plateThruFlaw:  usage ERROR"
        write(0,186)
        write(6,186)
  186  format(
     &   "#Usage:      plateThruFlaw Scale <histfile ",
     &   " >outputFile"//
...
C        Ok, its a number
  ...
  716      format("# Error too many data points:",i5,
     &        " recompile plateThruFlaw.f or reduce matfile data")
           stop
         endif
...
C        Ok, its a number
  ...
  616      format(" Error too many data points:",i5,
     &       " recompile plateThruFlaw.f or reduce dadnTable data")
           stop
         endif
...
C-------------- Initilize the Mm,Mb, Mkm,Mkb, fw storage -----------------------------

      ivec=0  ! =0 initilize,  =1  interpolate

C      aob=0.2   !  set to something
C     if(lactivateMmMb)then
C        call getLongMmMb(ivec,iret,aob,xMm,xMb)
C        if(iret.ne.0)then
C          write(0,*)"# Error from getLongMmMb() Init., Stopping..."
C          stop
C        endif
C     endif
...
C        call getLongMkmMkb(ivec,iret,aob,xMkm,xMkb)
C        if(iret.ne.0)then
C          write(0,*)"#Error: getLongMkmMkb() Initilize. Stopping."
C          stop
C        endif
C     endif
...
C     Create the  fw   table.
      ivec=0
      aow=0.2   !  set to something
      if(lactivatefw)then
        call fwThruFlaw(ivec,irec,aow,xfw)
        if(iret.ne.0)then
          write(0,*)"# Error:",iret," returned from fwThruFlaw()"
          write(0,*)"#   during initilization of table. Stopping..."
          stop
        endif
      endif
...
C        Ok, its a number
  ...
  916      format(" Error too many data points:",i5,
     &      "recompile plateThruFlaw.f or reduce load history data")
           stop
         endif
...
C     Ok,  previous copies do not exist. open it.
     ...
     &     form= "unformatted", status= "new", recl= 32)
      write(0,462)
      write(6,462)
...
C     total damage is large.
  ...
  119 format("#crk= #   NREV    a 90      NBLK    NACT    ",
     &       "lobj90     xfw")
C      write(6,120) nrev,totdam90,nblk,nact,lobj90,
C     &             nptt90
...
Cold      lobj= lobj*xkp(totdam,nrev)
      ...
      aow=totdam90/Width
      if(aow .gt. 0.5)then
         write(0,3002)totdam90,Width
         write(6,3002)totdam90,Width
 3002    format("#END: a/W > 0.5 : a= ",e14.7," W= ",e14.7)
         goto 9000   !stop
      endif


      if(lactivatefw)then   ! normally this would be true
         call fwThruFlaw(ivec,iret,aow,xfw)
C        i=ifix((aow-startaow)/deltaaow)+1   !this would be a bit faster.
C        xfw= fw(ifix((aow-startaow)/deltaaow)+1 )
        if(iret.ne. 0)then
          goto 9000  !stop
        endif
      endif
...
C     Assume: Bulge M=1,   km=1  ktm=1  ktb=1 Mm=Mb=1
...
C      lobj90=xfw*(xMkm90*xMm90*stsMembrane + xMkb90*xMb90*stsBending )
      lobj90=xfw*( stsMembrane + stsBending )
...
C     a very small da increment may not show up if crack is big.
      ...
      if(isavelevel .gt. 0)then
        write(6,120) nrev,totdam90,nblk,nact,lobj90,
     &             nptt90
...
C        write(6,*)nrev,totdam90,nblk,nact,
C     &             lobj90,aow,xfw,
C     &             stsMembrane,stsBending  !debug
...
C       Ok, Point(idat) on the curve is still smaller, keep going
 ...
 1000   continue
        ...
        write(0,*)"ERROR:plateThruFlaw:getStress2Strain.endofdoloop"
        iexit=1
...
C      end of hunting loop
 1000  continue
        ...
        write(0,*)" ERROR:plateThruFlaw:getloop.endofdoloop"
        iexit=1
...
C==============================================================
      ...
      SUBROUTINE fwThruFlaw(ivec,iret,aow,xfw)
      SAVE
C  ivec=0=create Matrix,  ivec=1=interpolate
C Create the table of fw values for a surface flaw as per BS7910 pg.185
C Data is precalculated and saved in a vector of 1000 pts,
C For run time just take the aow  and compute the point. If vector is
C big, no interpolation is needed. (we may do this in the mainline to
C save time on a subroutine branch).

C For plot output file :  (  write() i,aow,fw(i)  )
...
C       set xlabel "a/W"
C       set ylabel "fw"
C       set title "Plate Tru Flaw Finite Width Correction"
C       splot "fwData4Plot" u 1:2
...
C     (note that we may do the table lookup in mainline to save time)

C     ivec= 0   Initilize the limits for aow  and calc table
      nrows=1000
      maxfwdatarow=nrows  ! save in common block

      startaow=0.0
      endaow=0.5   ! 2.0 * a  is crack length.  W is total width
      deltaaow= (endaow-startaow)/ (float(nrows-1))

      pi=3.1415927
      write(6,10)nrows
   10 format("#fw # fwThruFlaw  starts..."/
     &   "#fw #ROWS= ",i4/
     &   "##fw a/W runs between 0.0  and 0.5 for fw calculation")


C     Loop for the c/W  variable.
      write(6,32)deltaaow
   32 format("#fw #deltaaow= ",f6.4)
      aow=startaow
C     aow gets incremented at end of do loop
      do 2000 iaow=1,nrows    ! ----------------------loop  cow----------------

      if(aow .ge. 0. .and. aow .le. 0.5)then
        ysecant=1.0/cos(pi*aow)   ! note that sec(a)=  1.0/cos(a)
        xfw=sqrt(ysecant)
        fw(iaow)=xfw
        write(6,50)aow,fw(iaow),iaow
   50   format("#fw #plotfw ",1x,e13.6,1x,e13.6,1x,i5)

        if(iaow .eq. 1 )then
          write(6,54)startaow
   54     format("#fw #STARTAOW= ",e14.7)
        endif
        if(icow.eq.maxfwdatarow )then
         write(0,2050)startaow,endaow,deltaaow
         write(6,2050)startaow,endaow,deltaaow
 2050    format("#fw #STARTAOW= ",e14.7/"#fw #ENDAOW= ",e14.7/
     &          "#fw #DELTAAOW= ",e14.7)
        endif
        go to 950
...
C     End of loop for aow
  950 continue
      aow=aow+deltaaow
 2000 continue

      write(0,*)"#fw  Finished creating fw Vector "
      write(6,*)"#fw  Finished creating fw Vector "
      iret=0
      return
...
C      if(debugfw)then
C        write(0,3010)
C 3010   format(" Enter a/W  : ",$)
C        read(5,*,end=9000)aow
C      endif

C     if not debug,  aow was passed into s/r
      if(aow .le.0. )then
        write(0,3020)aow
        write(6,3020)aow
 3020   format("#fw #Error: fwThruFlaw: a/W = ",e14.7/
     &         "#fw #Stopping...")
        iret=3020   !stop. give mainline a chance to exit gracefully.
        return
      endif

      i=ifix((aow-startaow)/deltaaow)+1

      if(i.lt.1 .or.  i.gt. maxfwdatarow) then
C        Out of bounds error
         write(0,3025)startaow,aow,endaow,i
         write(6,3025)startaow,aow,endaow,i
 3025    format("#ERROR: S/R fwThruFlaw: a/W  out of bounds:"/
     &    "#      start a/W=",f6.3," a/W= ",e14.7," end a/W= ",f6.3/
     &    "#      i=  ",i5/
     &    "# Stopping...")
...
C     Looks like all is ok to get the fw value from vector
      xfw= fw(i)
C      if(debugfw)then
C        write(0,*)aow,xfw
C      endif
