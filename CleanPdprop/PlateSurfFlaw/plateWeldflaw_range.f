C  plateWeldflaw_range.f   vers. 1.30   Notched Spec Crack Prop.  FAC jul.15 2013
      SAVE
C  Simple Range count only.  Based on plateWeldflaw.f with PushDown list removed.
C  Compile:  gfortran  -g -w -fbounds-check plateWeldflaw_range.f  -o plateWeldflaw_range
C  Usage:   plateWeldflaw_range  scaleFactor <loadHistory >outputFile
...
C vers. 1.30 Correct serious bug: calclation of dc/dN (was using "a")jul15 2013
C vers. 1.21 Corrected material file name error  Jan 6 2013
C vers. 1.2  Fixed typo #ACTIVATE_MkmMkb= near line 470. : 
C            caused inactive Mkm,Mkb,  Jan03 2013
C            Fix maxmkdatarow maxmkdatacol specifications.
C ves.  1.1  Correction: multiply  Y*sigma  by sqrt(pi*a)
C vers. 1.0  Remove Push-Down list code.  Just take Ranges as-is and 
C            compute damage.
C--------------------- Fork to plateWeldflaw_range.f Dec. 04 2012
...
C         The n...row etc parameters are set in the S/R.
      ...
      logical logiExist  !used in opening  rfadInput.rand   file
...
C---------------------------  Run time input data------------------
  184 continue
      write(6,185)
      write(0,185)
  185 format("# plateWeldflaw_range.f vers. 1.3 "/
     & "#Usage: plateWeldflaw_range  scale <histfile  >outfile"/)
...
C     and that iargc probably includes the "plateWeldflaw" as an arg.
      if( nargc .ne. 1)then
        write(0,*)" plateWeldflaw_range:  usage ERROR"
        write(6,*)" plateWeldflaw_range:  usage ERROR"
        write(0,186)
        write(6,186)
  186  format(
     &   "#Usage:      plateWeldflaw_range Scale <histfile ",
     &   " >outputFile"//
...
C      inquire(unit=5, NAME=stdinFileName)
      write(0,460)
      write(6,460)
  460 format("# Opening random access output file:  rfadInput.rand ...")
      inquire(file="rfadInput.rand", EXIST= logiExist)
      if(logiExist)then
        write(0,*)"#Error: an old copy of file  rfadInput.rand  exists."
        write(0,*)"#   You need to rename it or remove it before we"
        write(0,*)"#   can run the simulation.  Stopping now..."
        write(6,*)"#Error: an old copy of file  rfadInput.rand  exists."
...
C     Ok,  previous copies do not exist. open it.
      open(unit=60, file="rfadInput.rand", access="direct",
     &     form= "unformatted", status= "new", recl= 72)
      write(0,462)
      write(6,462)
  462 format("#Random Access output file: rfadInput.rand   opened.")
...
C     The interpretation of BS7910:2005  Annex S.2 pg 261
      ...
      lobj00=lobj00*rootPiC         !bug fix july 15 2013
...
C     We always write to the binary out file.
        nrecord=nrecord+1
...
C     Check for MAX. history repeats
  ...
  130   format("# Max no. of History Reps. Reached: nblk= ",i10/
     &      "#TOTDAM90=a= ",E14.7,"  TOTDAM00=c= ",e14.7," nrev= ",i9)
C       Write out the -ve number of the last rec.  that was written. 
C       All the other variables in this rec are end of test values.
        ndummy=-nrecord  !make -ve to make it unique.
        write(60,rec=1)ndummy,totdam90,totdam00,nblk,nact,
     &             lobj90,lobj00,xMm90,xMb90,xMm00,xMb00,
     &             xMkm90,xMkb90,xMkm00,xMkb00,xfw,
     &             stsMembrane,stsBending
        stop
...
C************ Going UP,  Tensile Direction **************************
 1135 continue
      dld90=lobj90-ldo90
      if(dld90.le.0.)then
        write(6,*)"#Error: 1135:dld90<=0.,lobj90,ldo90 = ",
